...
public:
    ...
    ParamServer()
    {
        ...
        nh.param<int>("liorf/imuType", imuType, 1);
        nh.param<float>("liorf/imuRate", imuRate, 90.0);
...
template<typename T>
void publishCloud_pose(const ros::Publisher& thisPub, const T& thisCloud, ros::Time thisStamp, std::string thisFrame,float pose[6])
{
    liorf::cloud_pos tempCloud;
    sensor_msgs::PointCloud2 tempCloud2;
    pcl::toROSMsg(*thisCloud, tempCloud2);
    tempCloud2.header.stamp = thisStamp;
    tempCloud2.header.frame_id = thisFrame;
    tempCloud.pose[0]=pose[0];
    tempCloud.pose[1]=pose[1];
    tempCloud.pose[2]=pose[2];
    tempCloud.pose[3]=pose[3];
    tempCloud.pose[4]=pose[4];
    tempCloud.pose[5]=pose[5];
    tempCloud.key_frame_cloud=tempCloud2;
    if (thisPub.getNumSubscribers() != 0)
        {
            thisPub.publish(tempCloud);
        }
...
pcl::PointCloud<pcl::PointXYZRGB>::Ptr  transformPoint(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloudIn, PointXYZIRPYT* transformIn)
    {
    ...
    // PointXYZIRPYT trans2PointTypePose(float transformIn[])
    // {
